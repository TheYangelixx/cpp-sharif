...
//        filledEllipseRGBA(m_renderer, x, y, r,r, 0, 0, 0, 255);
    ...
    while(flag)
    {
        ...
        {
            e->type=0;
            SDL_ShowCursor(SDL_ENABLE); // mouse zaher beshe
        ...
        else if(e->type == SDL_MOUSEMOTION) // age mouse tekoon khorde
        {
            e->type=0; // e pointer hast
            x=e->motion.x;
            y=e->motion.y;
            reDraw=true;
            SDL_ShowCursor(SDL_DISABLE); // mouse mahv beshe
        }
        else if(e->type == SDL_MOUSEBUTTONDOWN) // age mouse feshorde shode bood
        {
            if(e->button.button == SDL_BUTTON_LEFT) // age click chap bood
            {
                e->type=0;
                // sqaure ro bezarim roo back image
                SDL_SetRenderTarget(m_renderer, bkImg);
                //rectangleRGBA(m_renderer, x, y, x+r, y+r, 0, 255, 0, 255);    // dar mokhtasate x , y yek square bekesham
                checkerBoardColor(m_renderer, x, y, (x+y)/4, 5, 0xFF00FF00, 0xFF0000FF);
                SDL_SetRenderTarget(m_renderer, nullptr);
                reDraw=true; // baraye inke render copy kone
