...
int main() {
    ...
    copy(v9.begin(), v9.end(),v10.begin()); // jaygozin mikone dar index haye morde nazar
    ...
    // shakhte araye 2 bodi ba vector. yek sheskle namonazame dandanei dare
    vector<vector<int>> b(3); //3 radife vector. toole harkodoome vector ha moteghayere ke dar paeen meghdar dehi kardim
    b[0].push_back(12);
    b[0].push_back(13);
    b[1].push_back(10);
    b[2].push_back(40);
    ...
    b[2].push_back(7);
    b[2].push_back(8);
    b.push_back(vector<int>(4,0));//b.emplace_back(4,0); /*dare ye radife dige be vector ezafe mikone*/
    for(auto i:b[3]) //age adadi bedim ke bishtar az tedad radif haye vector bashe, hichi chap nemishe
        cout<<i<<" ";
    cout<<endl;


    vector<vector<int>> c(3, vector<int>(4,0)); // ye vector 2 bodi. dar har radif ye vector 4 taii az 0
    c[0][0]=9;
    for (int i = 0; i < c[2].size(); i++) {
        c[2][i] = i;
    }
    for(auto i:c[2]) //age adadi bedim ke bishtar az tedad radif haye vector bashe, hichi chap nemishe
        cout<<i<<" ";
    cout<<endl;
    ...
    // 1. Initialization: Create a 3x4 vector initialized with zeros
    vector<vector<int>> matrix(rows, vector<int>(cols, 0));

    // 4. Adding a new row dynamically
    matrix.push_back({10, 20, 30, 40, 50}); // Adds a row of size 4

    cout << "2D Vector Content:\n";
    // 3. Outputting the structure
